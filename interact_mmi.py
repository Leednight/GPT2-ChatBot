...
def top_k_top_p_filtering(logits, top_k=0, top_p=0.0, filter_value=-float('Inf')):
    ...
    if top_k > 0:
        ...
        for index, logit in enumerate(logits):
            indices_to_remove = logits[index] < torch.topk(logits[index], top_k)[0][..., -1, None]
            logits[index][indices_to_remove] = filter_value  # 对于topk之外的其他元素的logits值设为负无穷
    ...
    while True:
        try:
            ...
            for _ in range(args.max_len):
                ...
                for index, next_token_logit in enumerate(next_token_logits):
                    next_token_logits[index][tokenizer.convert_tokens_to_ids('[UNK]')] = -float('Inf')
